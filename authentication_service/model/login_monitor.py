    ...
    def authenticate_user(self, username, password):
        for user in self.__monitored_users___:
            if user.get_username() == username:
                ...
                # the login was success, if there are any login attempts, just reset it
                user_login_tracker.clear_fail_count()
                return True
        # in case the user does not exists, return fail
        return False
    
    def getMetrics(self):
        return {
            "max_user_login_attemps": self.__max_secuential_attemps_until_block__,
            "monitored_users": [user.get_status() for user in self.__monitored_users___]
        }
